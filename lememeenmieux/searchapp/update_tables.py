            ...
            if ("categories" in entry.keys() and "product_name" in entry.keys() and "nutrition_grade_fr" in entry.keys()):
                if (entry["categories"] == '' or entry["product_name"] == ''):
                    ...
                    if (lg==0):
                        ...
                        # Import information and location, if they exist
                        if ("stores" in entry.keys() and "purchase_places" and "url" in entry.keys()):
                            Product.objects.create(ProductName=prod_short, Grade=entry["nutrition_grade_fr"], Places=entry["purchase_places"], Stores=entry["stores"], Link=entry["url"], CatNum=cat_id)
                        else:
                        ...
                        if ("stores" in entry.keys() and "purchase_places" and "url" in entry.keys()):
                            Product.objects.create(ProductName=prod_short, Grade=entry["nutrition_grade_fr"], Places=entry["purchase_places"], Stores=entry["stores"], Link=entry["url"], CatNum=cat_query.id)
